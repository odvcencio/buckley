...
const SubmissionFallbackPrompt = `If tools are disabled, return only the same submission arguments as JSON, with this minimal shape: {"artifact":{"schema_version":"buckley.artifact/v1","artifact_id":"partial-handoff","kind":"subagent_result","status":"incomplete","title":"Partial handoff","summary":"Observed coverage and limitations","incomplete_reasons":["Specific unfinished work or uncertainty"]},"source_refs":["all"]}. Replace descriptive text with the actual result. Use ["all"] for all pages already captured in this run without copying IDs, explicit source_ref IDs for a subset, or [] if no captures exist; leave artifact.blocks and artifact.evidence_refs empty when selecting sources. Do not add an items field or use status complete. Do not claim coverage you have not verified. No surrounding prose or Markdown fences.`
