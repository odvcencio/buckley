...
func TestClassifyReviewCommandEvidence(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "nested Go module build", command: `go -C cgo_harness test -run '^$' .`, want: reviewEvidenceBuild},
		{name: "nested Go module tests", command: "go -C=cgo_harness test .", output: "ok  example.test/cgo_harness", want: reviewEvidenceTest},
		...
		{name: "Python aggregate compile", command: "python3 -m compileall -q .", want: reviewEvidenceBuild},
		{name: "Python exact file compile", command: "python3 -m py_compile scripts/check.py scripts/types.pyi", want: reviewEvidenceBuild},
		{name: "Python project check", command: "python3 scripts/check.py --check", output: "generated files are current", want: reviewEvidenceTest},
		...
		{name: "Windows go test name filter", command: "go.exe test -run TestOne ./pkg/model", output: "ok"},
		...
		{name: "Go nested module outside root", command: "go -C ../other test .", output: "ok"},
		{name: "Go nested module absolute", command: "go -C /tmp/other test .", output: "ok"},
		{name: "Go unsupported global flag", command: "go -mod=vendor test .", output: "ok"},
		{name: "Python compile without targets", command: "python3 -m py_compile"},
		{name: "Python compile arbitrary file", command: "python3 -m py_compile scripts/check.sh"},
		{name: "Python compile outside root", command: "python3 -m py_compile ../check.py"},
		{name: "Python checker without check mode", command: "python3 scripts/check.py"},
		{name: "Python checker with extra arguments", command: "python3 scripts/check.py --check --write"},
