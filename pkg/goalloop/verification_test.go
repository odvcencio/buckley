...
func TestDriveState_OnlyNewVerifiedEvidenceResetsNoProgress(t *testing.T) {
	for _, tc := range []struct {
		name       string
		previousID string
		current    taskstate.CompletionEvidenceState
		wantStreak int
	}{
		{"new evidence", "old", taskstate.CompletionEvidenceState{Version: 1, VerificationStatus: taskstate.VerificationPass, VerificationEvidenceID: "new"}, 0},
		{"same evidence", "old", taskstate.CompletionEvidenceState{Version: 1, VerificationStatus: taskstate.VerificationPass, VerificationEvidenceID: "old"}, 3},
		{"missing evidence", "old", taskstate.CompletionEvidenceState{Version: 1, VerificationStatus: taskstate.VerificationPass}, 3},
		{"failed check", "old", taskstate.CompletionEvidenceState{Version: 1, VerificationStatus: taskstate.VerificationFail, VerificationEvidenceID: "new"}, 3},
		{"unobserved state", "old", taskstate.CompletionEvidenceState{Version: 1, VerificationStatus: taskstate.VerificationPass, VerificationEvidenceID: "new", StateObservationFailed: true}, 3},
		{"unsupported version", "old", taskstate.CompletionEvidenceState{Version: 2, VerificationStatus: taskstate.VerificationPass, VerificationEvidenceID: "new"}, 3},
	} {
		t.Run(tc.name, func(t *testing.T) {
			drive := &driveState{
				lastEvidenceFingerprint: "unchanged",
				noProgressTurns:         2,
				completionEvidence: taskstate.CompletionEvidenceState{
					Version: 1, VerificationStatus: taskstate.VerificationPass, VerificationEvidenceID: tc.previousID,
				},
			}
			drive.observeHarnessProgress(TurnOutcome{EvidenceFingerprint: "unchanged", CompletionEvidence: tc.current})
			if drive.noProgressTurns != tc.wantStreak {
				t.Fatalf("no-progress streak=%d, want %d", drive.noProgressTurns, tc.wantStreak)
			}
		})
	}
}
