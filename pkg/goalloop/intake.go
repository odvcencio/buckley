...
func (l *Loop) Start(ctx context.Context, goal Goal) (*Intake, error) {
	...
	if _, err := l.ledger.Append(ctx, runledger.Event{
		...
		Payload: map[string]any{
			...
			"model_policy_version":              goal.ModelRequest.PolicyVersion,
			"model_policy":                      goal.ModelRequest.Policy,
			"model_policy_action":               goal.ModelRequest.PolicyAction,
			"model_policy_reason_code":          goal.ModelRequest.PolicyReasonCode,
			"model":                             goal.ModelRequest.Model,
			"reasoning_effort":                  goal.ModelRequest.ReasoningEffort,
			"retention_mode":                    goal.ModelRequest.RetentionMode,
			"openrouter_zdr":                    goal.ModelRequest.OpenRouterZDR,
			"data_collection":                   goal.ModelRequest.OpenRouterDataCollection,
			"workspace_license_file":            goal.ModelRequest.WorkspaceLicense.File,
			"workspace_license_id":              goal.ModelRequest.WorkspaceLicense.ID,
			"workspace_license_sha256":          goal.ModelRequest.WorkspaceLicense.SHA256,
			"workspace_license_manifest_sha256": goal.ModelRequest.WorkspaceLicense.ManifestSHA256,
		},
	}); err != nil {
