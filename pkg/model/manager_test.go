...
func TestInitializeFallsBackWhenModelsMissing(t *testing.T) {
	...
	// An unconfigured role being defaulted must still be reported loudly, and
	// that report must survive Initialize() so non-CLI callers (headless, ACP,
	// TUI) that don't watch stderr can surface it too.
	warnings := mgr.Warnings()
	if len(warnings) != 3 {
		t.Fatalf("expected 3 fallback warnings (planning/execution/review), got %d: %v", len(warnings), warnings)
	}
	for _, w := range warnings {
		if !strings.Contains(w, want) {
			t.Fatalf("expected warning to mention fallback model %q, got %q", want, w)
		}
	}
}

// TestEnsureModelErrorsOnUnavailableConfiguredModel locks in the fix for the
// "silent model fallback" landmine: a role that was explicitly configured to
// a model ID that isn't in any discovered catalog (typo, removed model, moved
// providers, ...) must fail loudly instead of being silently swapped for an
// arbitrary "first model in the provider's catalog".
func TestEnsureModelErrorsOnUnavailableConfiguredModel(t *testing.T) {
	cfg := &config.Config{
		Models: config.ModelConfig{
			Planning:        "p1/missing",
	...
	err := mgr.Initialize()
	if err == nil {
		t.Fatal("expected Initialize() to fail for an unavailable configured planning model, got nil error")
	}
	if !strings.Contains(err.Error(), "p1/missing") {
		t.Fatalf("expected error to name the unavailable model %q, got: %v", "p1/missing", err)
	}
	if !strings.Contains(err.Error(), "planning") {
		t.Fatalf("expected error to name the affected role %q, got: %v", "planning", err)
	}

	// The whole point: the config must NOT have been silently mutated to run a
	// different model than what was asked for.
	if cfg.Models.Planning != "p1/missing" {
		t.Fatalf("planning model must remain unchanged after failed validation, got %q", cfg.Models.Planning)
	}
}
...
// TestProviderRoutingKnownModelBeatsAlphabeticalOrderAndDefaultProvider locks
// in the fix for the primary landmine: providerForModel must consult the
// discovered model->provider map before falling back to ModelRouting
// prefixes, DefaultProvider, or alphabetical providerOrder. Here
// DefaultProvider is misconfigured to "ollama" (e.g. cleared/overridden) and
// "ollama" also sorts first alphabetically in providerOrder -- both of the
// old (buggy) tie-breakers would route every model to ollama. A model that is
// actually known to live on openrouter must still route to openrouter.
func TestProviderRoutingKnownModelBeatsAlphabeticalOrderAndDefaultProvider(t *testing.T) {
	cfg := &config.Config{
		Models: config.ModelConfig{
			DefaultProvider: "ollama", // misconfigured/cleared default
			FallbackChains:  map[string][]string{},
		},
		Providers: config.ProviderConfig{
			ModelRouting: map[string]string{}, // no prefix hints for "z-ai/" either
		},
	}
	...
	ollama := &stubProvider{
		id: "ollama",
		catalog: ModelCatalog{
			Data: []ModelInfo{{ID: "ollama/llama3", ContextLength: 8_000}},
		},
	}
	openrouter := &stubProvider{
		id: "openrouter",
		catalog: ModelCatalog{
			Data: []ModelInfo{{ID: "z-ai/glm-5.2", ContextLength: 128_000}},
		},
	}
	...
	mgr := &Manager{
		...
		providerOrder:  []string{"ollama", "openrouter"}, // alphabetical: ollama sorts first
		catalog:        make(map[string]ModelInfo),
		providerModels: make(map[string][]string),
		modelProviders: make(map[string]string),
	}
	...
	provider := mgr.providerForModel("z-ai/glm-5.2")
	if provider == nil || provider.ID() != "openrouter" {
		t.Fatalf("expected z-ai/glm-5.2 to route to openrouter (its known provider), got %v", provider)
	}

	// End-to-end: ChatCompletion must actually hit openrouter, not silently
	// hang/error against ollama at localhost.
	if _, err := mgr.ChatCompletion(context.Background(), ChatRequest{Model: "z-ai/glm-5.2"}); err != nil {
		t.Fatalf("ChatCompletion() error = %v", err)
	}
	if openrouter.lastRequest.Model != "z-ai/glm-5.2" {
		t.Fatalf("expected openrouter to receive the request, got model %q", openrouter.lastRequest.Model)
	}
	if ollama.lastRequest.Model != "" {
		t.Fatalf("did not expect ollama to receive any request, got model %q", ollama.lastRequest.Model)
	}
}
