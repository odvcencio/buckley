...
var factContractSpecs = []factContractSpec{
	...
	{domain: "runtime/protocol", purpose: "model behavior and task protocol compilation", fields: fields(
		field("model.class", "string"),
		field("model.tool_calls", "bool"),
		...
		field("model.parallel_call_reliability", "float64"),
		field("model.continuation_reliability", "float64"),
		field("model.sample_size", "int"),
		field("model.confidence", "float64"),
		field("task.phase", "string"),
		field("task.class", "string"),
		field("task.complexity", "int"),
		field("task.risk", "string"),
		field("task.parallelizable", "bool"),
		field("task.needs_artifact", "bool"),
		field("budget.utilization", "float64"),
		field("budget.latency_ms", "int64"),
		field("rollout.dynamic", "bool"),
		field("rollout.auto_code_mode", "bool"),
	)},
	{domain: "runtime/model_data_policy", purpose: "durable model data-retention and OSS-license gate", fields: fields(
		field("provider.name", "string"),
		field("privacy.retention_mode", "string"),
		field("privacy.zdr_enforceable", "bool"),
		field("privacy.data_collection", "string"),
		field("workspace.license_status", "string"),
		field("workspace.license_id", "string"),
		field("workspace.license_digest_match", "bool"),
	)},
	{domain: "runtime/goal_convergence", purpose: "durable goal turn convergence routing", fields: fields(
		field("termination.kind", "string"),
		field("workspace.state_changed", "bool"),
		field("turn.blocker_present", "bool"),
	)},
