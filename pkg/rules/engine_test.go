...
func TestEngine_EvalStrategy_ReviewPlan_AllSizes(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			wantIterations:   5,
			wantToolCalls:    0,
			wantVerification: 120,
			wantReserve:      90,
		},
		{
			name:             "large change stays completion-first",
			facts:            ReviewPlanFacts{FileCount: 21, DiffBytes: 75_000},
			wantSize:         "broad",
			wantReasoning:    2048,
			wantIterations:   6,
			wantToolCalls:    0,
			wantVerification: 180,
			wantReserve:      90,
		},
		{
			name:             "incomplete context stays completion-first",
...
func TestEngine_AllSubdirectoryStrategiesCompile(t *testing.T) {
	...
	strategies := map[string]string{
		...
		"permissions/builtin_defaults": "builtin_defaults_policy",
