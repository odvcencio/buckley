...
- APPROVE requires both Build and Tests to be PASS from focused local verification actually completed in the disposable workspace. Any FAIL, PENDING, NOT_RUN, UNAVAILABLE, or UNKNOWN state blocks approval.
- Build and Tests must each start with exactly one normalized state: PASS, FAIL, PENDING, NOT_RUN, UNAVAILABLE, or UNKNOWN. Do not write arbitrary prose in place of the state.
