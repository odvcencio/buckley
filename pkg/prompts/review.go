...
- Source-bearing native approval evidence must use separate standalone shell invocations at the snapshot root for Build and Tests, with no cd, echo/printf, chains, pipes, or redirections. Use classifiable forms such as focused go build or go test -run '^$' for Build and a package-scoped go test for Tests; nested Go modules may use go -C relative/module while the invocation stays at the snapshot root. A focused Go -run filter is allowed only with -v so the command event proves at least one test ran. Other supported forms are cargo build/check --workspace plus cargo test --workspace; npm/yarn/pnpm build/test scripts; or make build/test.
- For Python, use python3 -m compileall -q . for aggregate Build coverage or python3 -m py_compile path/to/changed.py for exact-file Build coverage. Tests may use pytest, or the exact python3 path/to/changed-checker.py --check form when that changed project script actually defines a read-only --check mode.
- For each changed language, required native build and test commands must use the same applicable toolchain and targets that, in aggregate across successful commands, cover every changed source path for that language.
