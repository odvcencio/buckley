...
func parseServeCommandOptions(args []string, ipcDefaults config.IPCConfig) (serveCommandOptions, error) {
	...
	assetPath := fs.String("assets", "", "optional external static UI override (served when --browser)")
	enableBrowser := fs.Bool("browser", ipcDefaults.EnableBrowser, "serve the embedded GoSX Mission Control UI (or --assets override)")
