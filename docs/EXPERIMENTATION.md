...
func init() {
    experimentRunCmd.Flags().StringSliceP("models", "m", nil,
        "Models to compare (e.g., -m claude-sonnet-4-5 -m gpt-5.2-codex-xhigh)")
...
|--------|---------|----------|
| `litellm/<name>` | `litellm/claude-sonnet-4-5` | LiteLLM proxy (uses proxy's model mapping) |
| `litellm/<provider>/<model>` | `litellm/anthropic/claude-sonnet-4-5` | LiteLLM with explicit provider routing |
| `ollama/<model>` | `ollama/qwen3-coder` | Local Ollama |
| `openrouter/<model>` | `openrouter/anthropic/claude-sonnet-4-5` | OpenRouter |
| Native | `gpt-5.2-codex-xhigh`, `claude-sonnet-4-5` | Auto-detected provider |
...
1. **`buckley experiment run -m claude-sonnet-4-5 -m ollama/qwen3-coder -p "Add dark mode"`** executes both in parallel with isolated worktrees
...
buckley experiment run "multilingual-i18n" \
    -m litellm/anthropic/claude-sonnet-4-5 \
...
│                                                     │   $0.0234 | 4.2k  │
│  ┌─────────────────────────────────────────────┐   │                   │
│  │ ✓ claude-sonnet-4-5  COMPLETED   $0.0234    │   │ ● gpt-5.2-codex-xhigh          │
│  │   Score: 100% | 4.2k tokens | 3m 42s        │   │   Running... 1m   │
│  │   ✓ test_pass  ✓ file_exists                │   │   $0.0156 | 3.1k  │
│  ├─────────────────────────────────────────────┤   │                   │
│  │ ◐ gpt-5.2-codex-xhigh             RUNNING                │   │ ○ qwen3-coder       │
│  │   $0.0156 | 3.1k tokens | 1m 23s            │   │   Pending         │
│  ├─────────────────────────────────────────────┤   │                   │
│  │ ○ ollama/qwen3-coder   PENDING                │   ├───────────────────┤
...
|----------|----------------|---------|
| Anthropic | `anthropic/<model>` | `anthropic/claude-sonnet-4-5-20241022` |
| OpenAI | `openai/<model>` | `openai/gpt-5.2-codex-xhigh` |
...
| Ollama (via LiteLLM) | `ollama/<model>` | `ollama/qwen3-coder` |
